...
export default function WorkshopCTA() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        title: "Registered!",
        description: "You're in. Check your email/WhatsApp for the link.",
      });
    } catch (err: any) {
  ...
  return (
    <section
      id="register"
      className="py-16 bg-gradient-to-br from-navy-900 to-navy-800"
    >
        ...
        <div className="grid md:grid-cols-2 gap-8 items-center">
          <div>
            ...
            <p className="mt-3 text-gray-300">
              Secure UPI/Card payments • Instant confirmation • Reminders via
              email/WhatsApp
            </p>
            ...
            {result?.success && (
              <div className="mt-6 p-4 rounded-xl border border-white/20 bg-white/10">
                <p className="mb-2">
                  Payment successful. Access granted for 48 hours.
                </p>
                {result.meetingUrl && (
                  <p className="mb-2">
                    Join link:{" "}
          ...
          <form
            onSubmit={onSubmit}
            className="bg-white rounded-2xl p-6 border-2 border-vlsi-100 text-navy-900 shadow-xl"
          >
            <div className="grid grid-cols-1 gap-4">
              ...
              <div>
                <label className="block text-sm mb-1" htmlFor="domainInterest">
                  Domain Interest
                </label>
                <select
                  id="domainInterest"
                  name="domainInterest"
                  value={form.domainInterest}
                  onChange={onChange}
                  className="w-full border rounded-md p-2"
                >
              ...
              <Button
                type="submit"
                disabled={loading}
                className="mt-2 bg-gradient-to-r from-vlsi-500 to-vlsi-600 hover:from-vlsi-600 hover:to-vlsi-700 text-white"
              >
