...
export default function DemoRegistration() {
  ...
  const refreshCaptcha = () => {
    setDisplayedCaptcha(generateCaptcha());
    setFormData((prev) => ({ ...prev, verificationCode: "" }));
    toast({ title: "Captcha Refreshed", description: "Please enter the new code." });
  };

  const handleInputChange = (field: keyof DemoRegistrationData, value: string | boolean) => {
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="grid lg:grid-cols-2 gap-12 items-center">
              ...
              <div
                ...
                onKeyDown={(e) => (e.key === "Enter" || e.key === " " ? (e.target as HTMLElement).click() : null)}
                ...
                {!videoUrl ? (
                  <form id="demo-form" onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div className="grid md:grid-cols-2 gap-4">
                      <Input placeholder="First Name" value={formData.firstName} onChange={(e) => handleInputChange("firstName", e.target.value)} className="bg-white/90 border-none rounded-full px-4 py-3" required />
                      <Input placeholder="Last Name" value={formData.lastName} onChange={(e) => handleInputChange("lastName", e.target.value)} className="bg-white/90 border-none rounded-full px-4 py-3" required />
                    </div>

                    <Input type="email" placeholder="Email" value={formData.email} onChange={(e) => handleInputChange("email", e.target.value)} className="bg-white/90 border-none rounded-full px-4 py-3" required />
                    <Input type="tel" placeholder="+91 Phone" value={formData.phone} onChange={(e) => handleInputChange("phone", e.target.value)} className="bg-white/90 border-none rounded-full px-4 py-3" required />

                    {/* Selects */}
                    <Select value={formData.courseCategory} onValueChange={(val) => handleInputChange("courseCategory", val)}>
                    ...
                    <Select value={formData.preferredLocation} onValueChange={(val) => handleInputChange("preferredLocation", val)}>
                      <SelectTrigger className="bg-white/90 border-none rounded-full px-4 py-3">
                    ...
                    <Textarea placeholder="Comments / Inquiry" value={formData.comments} onChange={(e) => handleInputChange("comments", e.target.value)} className="bg-white/90 border-none rounded-lg px-4 py-3 min-h-[80px]" />
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-2 gap-4 items-center">
                        ...
                        <Input placeholder="Enter the code above" value={formData.verificationCode} onChange={(e) => handleInputChange("verificationCode", e.target.value)} className="bg-white/90 border-none rounded-full px-4 py-3" required />
                      </div>
                      ...
                      <label className="flex items-center gap-2 text-sm text-gray-300 mt-3">
                        <input type="checkbox" checked={!!formData.whatsappOptIn} onChange={(e) => handleInputChange("whatsappOptIn", e.target.checked)} className="w-4 h-4 rounded" />
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="aspect-video bg-black rounded-lg overflow-hidden">
                      {toEmbedUrl(videoUrl).includes("youtube.com/embed") ? (
                        <iframe ref={(el) => (iframeRef.current = el)} id="azorix-demo-player" className="w-full h-full" src={toEmbedUrl(videoUrl)} allow="autoplay; encrypted-media" />
                      ) : (
                    ...
                    <div className="text-center">
                      <Button onClick={() => navigate("/cohort-preview")} className="bg-gradient-to-r from-vlsi-500 to-vlsi-600 hover:from-vlsi-600 hover:to-vlsi-700 text-white font-semibold px-6">
          ...
          <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="grid md:grid-cols-3 gap-8">
              <div className="text-center p-6 bg-vlsi-50 rounded-lg">
                <div className="w-16 h-16 bg-vlsi-600 rounded-full flex items-center justify-center mx-auto mb-4">📚</div>
              ...
              <div className="text-center p-6 bg-vlsi-50 rounded-lg">
                <div className="w-16 h-16 bg-vlsi-600 rounded-full flex items-center justify-center mx-auto mb-4">🛠️</div>
